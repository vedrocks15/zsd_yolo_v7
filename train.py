...
def train(hyp, opt, device, tb_writer=None):
    ...
    # saving versions of the model without any detection head.....
    ...
    ###########################################################
    # Loader to modified for ZSD based training.....
    ...
    model.class_weights = None #labels_to_class_weights(dataset.labels, nc).to(device) * nc  # attach class weights
    model.names = names
    ...
    if rank in [-1, 0]:
        ...
        if opt.data.endswith('coco.yaml') and nc == 80:  # if COCO
            for m in (last, best) if best.exists() else (last):  # speed, mAP tests
                results, _, _ = test_zsd.test(opt.data,
                                          batch_size=batch_size * 2,
                                          imgsz=imgsz_test,
                                          conf_thres=0.001,
                                          iou_thres=opt.iou_thres,
